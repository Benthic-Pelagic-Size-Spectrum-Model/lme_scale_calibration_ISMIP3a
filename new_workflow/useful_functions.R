...
gridded_sizemodel <- function(params, ERSEM_det_input = F, U_mat, V_mat, W_mat,
                              temp_effect = T, use_init = F){
  with(params, {
    ...
    detritivores <- predators <- array(0, c(numb_grid_cells, numb_size_bins,
                                            numb_time_steps+1))

    #vector to hold detritus biomass density (g.m-2) (W)
    detritus <- array(0, c(numb_grid_cells, numb_time_steps+1))

    #matrix for keeping track of growth (GG_v, GG_u) and reproduction (R_v, R_u)
    #from ingested food:
    reprod_det <- reprod_pred <- array(0, c(numb_grid_cells, numb_size_bins,
                                            numb_time_steps+1))
    growth_det <- growth_int_pred <- array(0, c(numb_grid_cells, numb_size_bins,
                                                numb_time_steps+1))

    #matrix for keeping track of predation mortality (PM_v, PM_u)
    pred_mort_det <- array(0, c(numb_grid_cells, numb_size_bins,
                                numb_time_steps+1))
    pred_mort_pred <- array(0, c(numb_grid_cells, numb_size_bins,
                                 numb_time_steps+1))

    #matrix for keeping track of catches (Y_v, Y_u)
    catch_det <- catch_pred <- array(0, c(numb_grid_cells, numb_size_bins,
                                          numb_time_steps+1))

    #matrix for keeping track of total mortality (Z_v, Z_u)
    tot_mort_det <- tot_mort_pred <- array(0, c(numb_grid_cells, numb_size_bins,
                                                numb_time_steps+1))
    ...
    #(Fvec_v, Fvec_u)
    fishing_mort_det <- fishing_mort_pred <- array(0,
                                                   c(numb_grid_cells,
                                                     numb_size_bins,
                                                     numb_time_steps+1))
    ...
    #output fisheries catches per yr at size (Y_u)
    catch_u <- fishing_mort_pred[, ind_min_fish_pred:numb_size_bins, 1]*
      predators[, ind_min_fish_pred:numb_size_bins, 1]
    catch_pred[, ind_min_fish_pred:numb_size_bins, 1] <-
      t(apply(catch_u, 1,
              function(i) i*size_bins_vals[ind_min_fish_pred:numb_size_bins]))

    #output fisheries catches per yr at size (Y_v)
    catch_v <- fishing_mort_det[, ind_min_fish_det:numb_size_bins, 1]*
      detritivores[, ind_min_fish_det:numb_size_bins, 1]
    catch_det[, ind_min_fish_det:numb_size_bins, 1] <-
      t(apply(catch_v, 1,
              function(i) i*size_bins_vals[ind_min_fish_det:numb_size_bins]))


    #Multipliers that do not require iteration
    x_u <- size_bins_vals[ind_min_fish_pred:numb_size_bins]*log_size_increase
    x_v <- size_bins_vals[ind_min_fish_det:numb_size_bins]*log_size_increase
    ...
    for(i in 1:numb_time_steps){
      if(i < numb_time_steps){
        ...
        prop_bu <- rowSums(t(apply(
          predators[, ind_min_fish_pred:numb_size_bins, i], 1,
          function(j) j*x_u)))
        prop_bv <- rowSums(t(apply(
      ...
      for(j in 1:(numb_grid_cells)){
        ...
        if(temp_effect){
          pel_tempeffect <- exp(c1-activation_energy/
                                  (boltzmann*(sea_surf_temp[j, i]+273)))
          ben_tempeffect <- exp(c1-activation_energy/
                                  (boltzmann*(sea_floor_temp[j, i]+273)))
        }else{
          pel_tempeffect <- 1
          ben_tempeffect <- 1
        }
        ...
        # yr-1
        #(f_pel)
        pred_growth <- (predators[j,,i]*log_size_increase)%*%(constant_growth)

        feed_rate_pel <- pel_tempeffect*
          as.vector(((feed_multiplier*pref_pelagic[j])*pred_growth)/
                      (1+handling*(feed_multiplier*pref_pelagic[j])*
                         pred_growth))

        # yr-1
        #(f_ben)
        detrit_growth <- (detritivores[j,,i]*
                            log_size_increase)%*%(constant_growth)

        feed_rate_bent <- pel_tempeffect*
          as.vector(((feed_multiplier*pref_benthos[j])*detrit_growth)/
                      (1+handling*(feed_multiplier*pref_benthos[j])*
                         detrit_growth))
        # yr-1
        #(f_det)
        detritus_multiplier <- (1/size_bins_vals)*hr_vol_filter_benthos*
          10^(log10_size_bins*metabolic_req_detritivore)*detritus[j, i]
        ...
        if(dynamic_reproduction){
          reprod_pred[j,,i] <- growth_prop*(1-(growth_pred+energy_pred))*
        ...
        sat_pel <- ifelse(feed_rate_pel > 0,
                          feed_rate_pel/(
                            (feed_multiplier*pref_pelagic[j])*pred_growth),
                          0)
        # yr-1 (PM_u)
        pred_mort_pred[j,,i] <- as.vector(
          (pref_pelagic[j]*hr_volume_search*met_req_log10_size_bins)*
            (predators[j,,i]*sat_pel*log_size_increase)%*%(constant_mortality))

        # yr-1 (Z_u)
        tot_mort_pred[j,,i] <- pred_mort_pred[j,,i]+
          pel_tempeffect*other_mort_pred+senes_mort_pred+fishing_mort_pred[j,,i]

        # Benthos growth integral
        # yr-1 (GG_v)
        growth_det[j,,i] <- high_prop*growth_detritus*feed_rate_det

        #reproduction
        # yr-1 (R_v)
        if(dynamic_reproduction){
          reprod_det[j,,i] <- high_prop*
            (1-(growth_detritus+energy_detritivore))*
            (feed_rate_det)
        }
        ...
        pred_mort_det[, i] <- ifelse(sat_ben > 0,
                                     as.vector(
                                       (pref_benthos[j]*hr_volume_search*
                                          met_req_log10_size_bins)*
                                         (predators[j,,i]*sat_ben*
                                            log_size_increase)%*%(constant_mortality)),
                                     0)

        # yr-1 (Z_v)
        tot_mort_det[j,,i] <- pred_mort_det[j,,i]+
          ben_tempeffect*other_mort_det+senes_mort_det+fishing_mort_det[j,,i]

        #total biomass density eaten by pred (g.m-2.yr-1)
        eatenbypred <- size_bins_vals*feed_rate_pel*
          predators[j,,i]+size_bins_vals*feed_rate_bent*predators[j,,i]


        #detritus output (g.m-2.yr-1)
        # losses from detritivore scavenging/filtering only:
        output_w <- sum(size_bins_vals*feed_rate_det*detritivores[j,,i]*
                          log_size_increase)
        ...
        if(!ERSEM_det_input){
          ...
          if(detritus_coupling){
            ...
            input_w <- (sinking_rate[j,i]*
                          (sum(defbypred[ind_min_pred_size:numb_size_bins]*
                                 log_size_increase)+
                             sum(pel_tempeffect*other_mort_pred*predators[j,,i]*
                                   size_bins_vals*log_size_increase) +
                             sum(pel_tempeffect*senes_mort_pred*predators[j,,i]*
                                   size_bins_vals*log_size_increase)) +
                          (sum(ben_tempeffect*other_mort_det*detritivores[j,,i]*
                                 size_bins_vals*log_size_increase) +
                             sum(ben_tempeffect*senes_mort_det*
                                   detritivores[j,,i]*size_bins_vals*
                                   log_size_increase)))
          }else{
            input_w <- sum(ben_tempeffect*other_mort_det*detritivores[j,,i]*
                             size_bins_vals*log_size_increase)+
              sum(ben_tempeffect*senes_mort_det*detritivores[j,,i]*
                    size_bins_vals*log_size_increase)
          }
          ...
          detritus[j, i+1] <- detritus[j, i]+dW*timesteps_years
        }
        ...
        Ai_u[idx] <- (1/log(10))*-growth_int_pred[j, idx-1, i]*
          timesteps_years/log_size_increase
        Bi_u[idx] <- 1+(1/log(10))*growth_int_pred[j, idx, i]*
          timesteps_years/log_size_increase+tot_mort_pred[j, idx, i]*
          timesteps_years
        Si_u[idx] <- predators[j, idx, i]
        
        # Boundary condition at upstream end 
        Ai_u[ind_min_pred_size] <- 0
        Bi_u[ind_min_pred_size] <- 1
        Si_u[ind_min_pred_size] <- predators[j, ind_min_pred_size, i]
        
        # Invert matrix
        #recruitment at smallest consumer mass
        #continuation of plankton hold constant  
        predators[j, 1:ind_min_pred_size, i+1] <-
            ui0[j, i]*10^(slope_phy_zoo[j, i]*
                            log10_size_bins)[1:ind_min_pred_size]
        
        # apply transfer efficiency of 10% *plankton density at same size
        # reproduction from energy allocation
        if(dynamic_reproduction){
          predators[j, ind_min_pred_size, i+1] <- 
            predators[j, ind_min_pred_size, i]+
            (sum(reprod_pred[j, (ind_min_pred_size+1):numb_size_bins, i]*
                   size_bins_vals[(ind_min_pred_size+1):numb_size_bins]*
                   predators[j, (ind_min_pred_size+1):numb_size_bins, i]*
            ...
            (growth_int_pred[j, ind_min_pred_size, i])*
            predators[j, ind_min_pred_size, i]-timesteps_years*
            tot_mort_pred[j, ind_min_pred_size, i]*
            predators[j, ind_min_pred_size, i]
        }
        
        #main loop calculation
        for(sz in (ind_min_pred_size+1):(numb_size_bins)){
          predators[j, sz, i+1] <- (Si_u[sz]-Ai_u[sz]*
        ...
        Si_v[ind_min_detritivore_size] <-
          detritivores[j, ind_min_detritivore_size, i]
        
        #invert matrix
        #recruitment at smallest detritivore mass
        #hold constant continuation of plankton with sinking rate multiplier
        detritivores[j, 1:ind_min_detritivore_size, i+1] <-
          detritivores[j, 1:ind_min_detritivore_size, i]
        
        # apply a very low of transfer efficiency 1%* total biomass of detritus
        #divided by minimum size
        if(dynamic_reproduction){
          detritivores[j, ind_min_detritivore_size, i+1] <-
            detritivores[j, ind_min_detritivore_size, i]+
            sum(reprod_det[j, (ind_min_detritivore_size+1):numb_size_bins, i]*
                  size_bins_vals[(ind_min_detritivore_size+1):numb_size_bins]*
                  detritivores[j, (ind_min_detritivore_size+1):numb_size_bins, i]*
                  log_size_increase)*timesteps_years/
            (log_size_increase*size_bins_vals[ind_min_detritivore_size])-
            (timesteps_years/log_size_increase)*(1/log(10))*
            (growth_det[j, ind_min_detritivore_size, i])*
            detritivores[j, ind_min_detritivore_size, i]-
            timesteps_years*tot_mort_det[j, ind_min_detritivore_size, i]*
            detritivores[j, ind_min_detritivore_size, i]
        }
        
        # loop calculation
        for(sz in (ind_min_detritivore_size+1):(numb_size_bins)){
          detritivores[j, sz, i+1] <- (Si_v[sz]-Ai_v[sz]*
                                         detritivores[j, sz-1, i+1])/Bi_v[sz]
        }
        
        # Update Fmort
        if(i < numb_time_steps){
          fishing_mort_pred[j, ind_min_fish_pred:numb_size_bins, i+1] <-
            fish_mort_pred*effort[j, i+1]
          fishing_mort_det[j, ind_min_fish_det:numb_size_bins, i+1] <-
            fish_mort_detritivore*effort[j, i+1]
        }
        
        #output fisheries catches per yr at size
        catch_pred[j, ind_min_fish_pred:numb_size_bins, i+1] <-
          fishing_mort_pred[j, ind_min_fish_pred:numb_size_bins, i+1]*
          predators[j, ind_min_fish_pred:numb_size_bins, i+1]*
          size_bins_vals[ind_min_fish_pred:numb_size_bins]
        #output fisheries catches per yr at size
        catch_det[j, ind_min_fish_det:numb_size_bins, i+1] <-
          fishing_mort_det[j, ind_min_fish_det:numb_size_bins, i+1]*
          detritivores[j, ind_min_fish_det:numb_size_bins, i+1]*
