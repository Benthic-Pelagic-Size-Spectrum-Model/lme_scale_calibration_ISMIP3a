...
if __name__ == '__main__':
    ...
    # p = xr.open_dataarray(glob(os.path.join(out_folder, 'predators_*_1854-12.nc'))[0])
    #Subset predator data for the period you want to run the model
    # predators = predators.sel(time = slice('1849-04', '1849-12'))
    ...
    # e_start = xr.open_dataarray(glob(os.path.join(out_folder, 'effort_*_1855-01.nc'))[0])
    # #Subset effort data from the month after DBPM begins
    # effort = effort.sel(time = slice('1855-02', '1860-01'))
    # #Combine both data arrays
    ...
    # pel_tempeffect = pel_tempeffect.sel(time = slice('1855-01', '1859-12'))
    # ben_tempeffect = ben_tempeffect.sel(time = slice('1855-01', '1859-12'))
    
    #Sinking rate
    sinking_rate = xr.open_zarr(
        glob(os.path.join(base_folder, 'gridded', model_res, '*_spinup_er_*'))[0])['export_ratio']
    
    # sinking_rate = sinking_rate.sel(time = slice('1855-01', '1859-12'))
