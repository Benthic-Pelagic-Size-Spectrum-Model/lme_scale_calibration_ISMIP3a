...
if __name__ == '__main__':
    ...
    dynamic_vars = ['input-w20m', 'expc-bot', 'er', 'lphy', 'sphy', 'tob',
                    'tos', 'ocean-temp-weighted']
    ...
    for res in resolutions:
        ...
        f_out = (si_files.replace('ctrlclim', 'spinup').replace('1961', '1841').
            replace('2010', '1960'))
        uf.gridded_spinup(si_files, '1961-01', '1980-12', spinup_period, 
                          file_out = f_out)
        #Create files for the stable spinup period from sea ice concentration
        #Load area of grid cells
        area = xr.open_zarr(glob(os.path.join(gfdl_folder, 
                                              '*areacello*'))[0])['cellareao']
        #Load sea ice concentration
        sic = ((xr.open_zarr(glob(os.path.join(
            gfdl_folder, f'gfdl-*ctrlclim_siconc_*'))[0])['siconc']).
            where(np.isfinite(area)))
        #Getting SIC spinup
        da = uf.gridded_spinup(sic, '1961-01', '1961-12', stable_spin,
                               mean_spinup = True)
        ...
        #(45N) is thelowest latitude area where sea ice forms each winter 
        ...
        # Earth Observatory
        da_mask_south = xr.where(da_mask.lat <= -52, da_mask, 
                                 False).cumsum('lat')
        ...
        if smoothing is not None:
            gfdl_folder = os.path.join(base_dir, 
                                       f'global_gridded-{smoothing}_zarr', res)
        ...
        #Save outputs
        int_out = (glob(os.path.join(gfdl_folder, 
                                     f'gfdl-*ctrlclim_intercept_*monthly*'))[0].
            replace('ctrlclim', 'stable-spin').replace('1961', '1741').
            replace('2010', '1840'))
        intercept.to_zarr(int_out, consolidated = True, mode = 'w')

        slo_out = (glob(os.path.join(gfdl_folder, 
                                     f'gfdl-*ctrlclim_slope_*monthly*'))[0].
            replace('ctrlclim', 'stable-spin').replace('1961', '1741').
            replace('2010', '1840'))
