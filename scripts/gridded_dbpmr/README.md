...
export DBPM_DATA=$PWD/DBPM_data            # biology-params init JSONs (equilibrium_runs/, see manifest)
L=3
...
| `DBPM_data/equilibrium_runs/init_dbpm_nonspatial_fao_lme-<L>_searchvol_*.json` | biology params (`$params`: global constants + depth) read by steps 3–4; path set via `DBPM_DATA` (default `DBPM_data`) | 0-D equilibrium/initialisation runs (`DBPM_dev/equilibrium_runs`, pipeline stage 05) | no (per-region) |
...
├── DBPM_data/equilibrium_runs/     # biology-params init JSONs      (DBPM_DATA)
...
| 3 | `gridded_calib.R` | percell_c parquet, region parquet, mask, `gfw_static.csv`, `siconc_lme<L>.csv`, `calib_A3/lme<L>.rds`, `DBPM_data/…/init_…json` (biology params, `$DBPM_DATA`) | `gridded_calib_lme<L>.rds` (fitted q) | ~2–3 h |
