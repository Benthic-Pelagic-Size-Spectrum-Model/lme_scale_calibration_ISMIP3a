...
# faster using pbsapply, in the LHSsearch pbapply has cl=6 which uses cluster to run in parallel, but here it is run sequentially.
lmenum=66
lmes<-t(pbsapply(X=1:lmenum,LHSsearch,iter=100))
saveRDS(lmes,"bestvals_LMEs_round2.RDS")

############### Make plots

#### Check other model performance indicators using the above estimates
bestvals<-data.frame(readRDS("bestvals_LMEs.RDS"))

# add column for correlation:
bestvals$cor<-rep(0,lmenum)

pdf("CalibrationPlots.pdf",height = 6, width = 8)
