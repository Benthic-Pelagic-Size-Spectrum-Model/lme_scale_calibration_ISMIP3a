...
plotgridbyLME <- function(LMEnumber){
  ...
  # load outputs 
  LME_path <- file.path("/rd/gem/private/fishmip_outputs/ISIMIP3a/DBPM/obsclim",
                        paste0("LME_", LMEnumber, "_output"))
  grid_results <- readRDS(file.path(LME_path, "grid_results.rds"))
  ...
  load(file.path(LME_path, "/grid_results_inputs_params.RData"))
  ...
  ## CN- is this necessary as you already saved gridded_output and have out
  #here?? 
  ...
  # colnames(out)
  head(out[, c("t", "Totalcatch", "TotalVcatch", "TotalUcatch", "TotalVbiomass", 
               "TotalUbiomass")])

  # download world map - SAVE A COPY IN SHARED DRIVE
  world <- ne_countries(category = "cultural", type = "admin_0_countries",
  ...
  p1 <- ggplot(biom_decade_avg)+
    ...
    scale_x_continuous(name = "Longitude", 
                       breaks = seq(lon_ext[1], lon_ext[2], by = 6))+
  ...
  spectra_grid_avg <- array(NA, 
                            dim = c(length(lat_list),
                                    dim(spectra_decade_avg)[2:3]),
  ...
  for (iCell in names(lat_list)) {
    ...
    if(length(dim(tempBiom)) == 3){
      ...
      avgBiom <- apply(tempBiom, c(2, 3), mean) 
    }else{
      avgBiom <- tempBiom
    }
  ...
  name <- ifelse(f.effort == FALSE, "/plots_no_fishing.pdf",
                 "/plots_fishing.pdf")
