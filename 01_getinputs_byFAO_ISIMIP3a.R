...
# 1 FAO aggregated - for calibration, using 0.25deg inputs only and focusing on obsclim
# 2 FAO gridcell level at 1deg and 0.25 deg - for model running, need both inputs and both scales 
...
calc_inputs_FAO<-function(file_name_obs, file_name_crtl, file_path_crtl, file_path_obs){
  ...
  area_frame <- data.frame("lon" = rep(seq(-179.875,179.875, by = 0.25), by = 720), 
                           ...
                           "area_m2" = 1e6*as.vector(t(as.matrix(area(raster(nrows = 720, ncol = 1440))))))
  
  fao_crtl_raw <- data.table::fread(file.path(file_path_crtl, file_name_crtl), header = FALSE)
  ...
  fao_crtl <- left_join(fao_crtl, area_frame, by = c("lat" = "lat", "lon" = "lon"))
  fao_crtl <- fao_crtl %>% na.omit() # Some boundary cells are NAs, remove (very small number)
  
  # then work on OBSERVED, OPEN AND RESHAPE DATA
  fao_raw <- data.table::fread(file.path(file_path_obs, file_name_obs), header = FALSE)
  ...
  rm("fao_crtl_raw", "fao_raw", "dat1", "dates", "all_dat", "years", "months", "col_names")
  ...
  if (variable == "deptho_m"){
    ...
    weighted_mean_obs<-fao %>% 
      ...
      mutate(FAO = str_extract(file_name_obs,"(?<=FAO-LME-).+(?=_monthly)")) ## Change here! From LME to FAO, RFH Sep. 2023
    ...
    weighted_mean_crtl<-fao_crtl %>% 
      ...
      summarise(weighted_mean_crtl = weighted.mean(value, area_m2),
                area_km2 = sum(area_m2)/1e6) %>% 
      ungroup()  %>% 
      mutate(FAO = str_extract(file_name_crtl,"(?<=FAO-LME-).+(?=_monthly)"), ## Change here! From LME to FAO, RFH Sep. 2023
             Month = str_extract(Date,"[[:upper:]]+[[:lower:]]+"),
             Year = str_extract(Date, "\\d+"), 
             Date = lubridate::my(paste(Month,Year, sep = "." ))) %>% 
      arrange(Date)
    
    # # to plot METHOD 1 and compare with METHOD 2 -->>  DELETED BY RFH (SEP 2023) TO TIDY CODE 
    ...
    # reorder columns 
    weighted_mean_crtl_final<-weighted_mean_crtl_final[,c("FAO", "Date", "Year", "Month", "weighted_mean_crtl", "area_km2")]
    ...
    # reorder columns 
    weighted_mean_obs_final<-weighted_mean_obs_final[,c("FAO", "Date", "Year", "Month", "weighted_mean_obs", "area_km2")] ## Change here! From LME to FAO, RFH Sep. 2023
...
calc_inputs_all_FAO<-function(this_FAO){
  ...
  fao_obs<-list.files(file_path_obs, pattern = this_FAO_new) #, full.names = TRUE) # this adds the whole path
  ...
  this_destination_path_obs <- paste0("/rd/gem/private/fishmip_inputs/ISIMIP3a/processed_forcings/fao_inputs/obsclim/0.25deg/", "observed_FAO_", this_FAO, ".csv")
  this_destination_path_ctrl <- paste0("/rd/gem/private/fishmip_inputs/ISIMIP3a/processed_forcings/fao_inputs/ctrlclim/0.25deg/", "control_FAO_", this_FAO, ".csv")
...
newly_written_files_observed <- list.files("/rd/gem/private/fishmip_inputs/ISIMIP3a/processed_forcings/fao_inputs/obsclim/0.25deg", pattern = "observed", full.names = TRUE)

# pick one randomly and check 
# map(newly_written_files_observed[[8]], fread)

# combine files 
combined_FAO_inputs <- rbindlist(mclapply(X = newly_written_files_observed, FUN = fread, mc.cores = 40))
FAO_area <- combined_FAO_inputs %>% dplyr::group_by(FAO) %>% dplyr::select(FAO, area_km2) %>% unique()
...
combined_FAO_inputs<-combined_FAO_inputs %>% 
  dplyr::mutate(sphy = `phypico-vint`, 
...
effort<-data.table::fread("/rd/gem/private/users/yannickr/DKRZ_EffortFiles/effort_isimip3a_histsoc_1841_2010.csv")
effort_FAO <- effort %>% dplyr::filter(LME == 0)
...
FAO_area<-combined_FAO_inputs %>% 
  dplyr::select(FAO, area_m2) %>% # this is FAO_area total area
  dplyr::mutate(deptho_m = 200) %>% # all depths are almost definitely > 200m in FAO regions excluding LMEs
  unique()
...
catch_FAO <- catch %>% dplyr::filter(LME == 0)

DBPM_FAO_catch_input<-catch_FAO %>% 
  mutate(catch_tonnes = Reported+IUU) %>% 
  group_by(Year, fao_area) %>% 
  summarize(catch_tonnes = sum(catch_tonnes), .groups = "drop") %>% # catch is in tonnes, checked in FishingEffort Rproject, 
...
plot_df<-split(DBPM_FAO_effort_catch_input, DBPM_FAO_effort_catch_input$fao_area)
...
DBPM_FAO_climate_inputs_renamed<-DBPM_FAO_climate_inputs %>% left_join(FAO_area, by = c("FAO" = "FAO")) %>%
  mutate(deptho_m = 200, area_m2 = area_km2*1e6) %>% 
  dplyr::select(FAO, Date, deptho_m, area_m2, `expc-bot`, tob, tos, sphy, lphy) %>% 
  dplyr::rename(t = Date, depth = deptho_m, sbt = tob, sst = tos, expcbot = `expc-bot`)

DBPM_FAO_climate_inputs_renamed<-DBPM_FAO_climate_inputs_renamed[,c("FAO", "t", "lphy", "sphy", "sbt", "sst", "depth", "area_m2", "expcbot")]
...
DBPM_FAO_climate_inputs_slope<-DBPM_FAO_climate_inputs_renamed %>%
  ...
  summarise(er = getExportRatio(sphy,lphy,sst,depth),
            ...
            slope = GetPPIntSlope(sphy,lphy,mmin=10^-14.25,mmid=10^-10.184,mmax=10^-5.25,depth,output="slope"), .groups = "drop") %>% 
  ungroup() %>% 
  relocate("FAO","t","sst","sbt","er","intercept","slope", "sphy", "lphy", "depth", "area_m2","expcbot")
...
fwrite(x = DBPM_FAO_climate_inputs_slope, file.path("/rd/gem/private/fishmip_inputs/ISIMIP3a/processed_forcings/fao_inputs/obsclim/0.25deg/", "DBPM_FAO_climate_inputs_slope.csv"))
fwrite(x = DBPM_FAO_effort_catch_input, file.path("/rd/gem/private/fishmip_inputs/ISIMIP3a/processed_forcings/fao_inputs/obsclim/0.25deg/", "DBPM_FAO_effort_catch_input.csv"))

# print into public folder too
fwrite(x = DBPM_FAO_climate_inputs_slope, file.path("/rd/gem/public/fishmip/ISIMIP3a/InputData/DBPM_fao_inputs/obsclim/0.25deg/", "DBPM_FAO_climate_inputs_slope.csv"))
fwrite(x = DBPM_FAO_effort_catch_input, file.path("/rd/gem/public/fishmip/ISIMIP3a/InputData/DBPM_fao_inputs/obsclim/0.25deg/", "DBPM_FAO_effort_catch_input.csv"))
...
#### 1. Define main function: for each FAO, and each variable, apply the calc_inputs_FAO function -----

# 1A. calc_inputs_FAO()
# see function description above - this is adjusted for gridcell level but names are kept - hence e.g. weighted_mean_obs
calc_inputs_FAO<-function(file_name_obs, file_name_crtl, file_path_crtl, file_path_obs, degree){
  
  # extract variable name
  if(degree == "1deg"){
  variable<-str_match(file_name_obs, "gfdl-mom6-cobalt2_obsclim_\\s*(.*?)\\s*_60arcmin")[2]
  ...
  area_frame <- data.frame("lon" = rep(seq(-179.5,179.5, by = 1), by = 180), 
                           ...
                           "area_m2" = 1e6*as.vector(t(as.matrix(area(raster(nrows = 180, ncol = 360))))))
  }
  ...
  fao_crtl <- left_join(fao_crtl, area_frame, by = c("lat" = "lat", "lon" = "lon"))
  fao_crtl <- fao_crtl %>% na.omit() # A small handful of cells on edge of domain have nas in most fao regions. Consistent across all variables
  ...
  # NOTE - through the code "weighed_mean" is only because of code recycling from fao-aggregated level approach above 
  ...
  if (variable == "deptho_m"){
    ...
    weighted_mean_crtl<-fao_crtl %>% 
      gather(key = "Date",
             value = "value",
             -c("lat", "lon","area_m2")) %>% 
      dplyr::mutate(FAO = str_extract(file_name_crtl,"(?<=FAO-LME-).+(?=_monthly)"),
             Month = str_extract(Date,"[[:upper:]]+[[:lower:]]+"),
             Year = str_extract(Date, "\\d+"), 
             Date = lubridate::my(paste(Month,Year, sep = "." ))) %>% 
      arrange(Date) %>% 
    ...
    # reorder columns 
    weighted_mean_crtl<-weighted_mean_crtl[,c("lat","lon","FAO", "Date", "Year", "Month", "value")]
    ...
    # reorder columns 
    weighted_mean_obs<-weighted_mean_obs[,c("lat","lon", "FAO", "Date", "Year", "Month", "value")]
  ...
  return(list(weighted_mean_obs_final = weighted_mean_obs, weighted_mean_crtl_final = weighted_mean_crtl)) 
  
}

## grid cell level FAO are too big to save with spin up, need to process first then add spinup
...
calc_inputs_all_FAO<-function(this_FAO, degree){
  ...
  file_path_obs<- paste("/rd/gem/private/fishmip_inputs/ISIMIP3a/fao_inputs/obsclim/" , degree, sep = "")
  file_path_crtl<- paste("/rd/gem/private/fishmip_inputs/ISIMIP3a/fao_inputs/ctrlclim/", degree, sep = "")
...
#  tic()
 ...
 # toc() #
  ...
  output_crtl_all_variables<-output_crtl_all_variables[,c(1,2,3,4,5,6,7,14,21,28,35)]
  output_crtl_all_variables<-left_join(output_crtl_all_variables, area_frame, by = c("lat" = "lat", "lon" = "lon"))
  
  if(!("depth" %in% colnames(output_crtl_all_variables))){ # If no depth variable
  output_crtl_all_variables<- output_crtl_all_variables %>% arrange(lat, lon, Date) %>% dplyr::mutate(depth = 200)
  }
  #toc() # 4 sec
  ...
  output_crtl_all_variables<-output_crtl_all_variables %>% 
    dplyr::select(lat, lon, FAO, Date, Year, Month, depth, area_m2, `expc-bot`, tob, tos, sphy, lphy) %>% 
    dplyr::rename(lat = lat, lon = lon, t = Date, sbt = tob, sst = tos, expcbot = `expc-bot`)
  
  output_crtl_all_variables<-output_crtl_all_variables[,c("lat","lon","FAO", "t", "Year", "Month", "lphy", "sphy", "sbt", "sst", "depth", "area_m2", "expcbot")]
  
  # # dplyr method, WORKS FOR ME - RFH SEP 2023
  output_crtl_all_variables<-output_crtl_all_variables %>% ungroup() %>%
    dplyr::mutate(er = getExportRatio(sphy,lphy,sst,depth),
                  er = ifelse(er<0,0, ifelse(er>1,1,er)),
                  er = round(er, 3),
                  sst = round(sst, 3),
                  sbt = round(sbt, 3),
                  intercept = round(GetPPIntSlope(sphy,lphy,mmin=10^-14.25,mmid=10^-10.184,mmax=10^-5.25,depth,output="intercept"), 3),
                  slope = round(GetPPIntSlope(sphy,lphy,mmin=10^-14.25,mmid=10^-10.184,mmax=10^-5.25,depth,output="slope"),3),
                  sphy = round(sphy, 5),
                  lphy = round(lphy, 5)) %>% 
    relocate("lat","lon","FAO","t","sst","sbt","er","intercept","slope", "sphy", "lphy", "depth", "area_m2","expcbot") 
...
#  1 3.638025e-09 1961   Jan
  # intercept = -0.509, round(GetPPIntSlope(0.05052,0.01246,mmin=10^-14.25,mmid=10^-10.184,mmax=10^-5.25,200,output="intercept"), 3)
  # slope = -1.123, round(GetPPIntSlope(0.05052,0.01246,mmin=10^-14.25,mmid=10^-10.184,mmax=10^-5.25,200,output="slope"), 3)
  # er = 0.06, round(getExportRatio(0.05052,0.01246,28.407,200), 3)
  
  ## Save output
  this_destination_path_ctrl <- paste0("/rd/gem/private/fishmip_inputs/ISIMIP3a/processed_forcings/fao_inputs_gridcell/ctrlclim/", degree, "/control_historical_FAO_", this_FAO, "_", degree, ".csv")
  fwrite(x = output_crtl_all_variables, file = file.path(this_destination_path_ctrl))
  ...
  kk <- data.table::fread(this_destination_path_ctrl)
  ...
  for(i in 1:num_times){
    curr_spin <- spinup %>% dplyr::mutate(Year = Year - (120-(20*(i-1))))
  ...
  spinup <-spinup %>% dplyr::mutate(t = lubridate::my(paste(Month,Year, sep = "." ))) %>%
          dplyr::select(-c(Month, Year))
  
  ### JUST SAVE SPINUP
  this_destination_path_ctrl <- paste0("/rd/gem/private/fishmip_inputs/ISIMIP3a/processed_forcings/fao_inputs_gridcell/ctrlclim/", degree, "/control_spinup_FAO_", this_FAO, "_", degree, ".csv")
...
#  this_destination_path_ctrl <- paste0("/rd/gem/private/fishmip_inputs/ISIMIP3a/processed_forcings/fao_inputs_gridcell/ctrlclim/", degree, "/control_FAO_", this_FAO, "_", degree, ".csv")
#  fwrite(x = kk, file = file.path(this_destination_path_ctrl))
#  rm("kk")
  
  ## Delete ctrlclim file for current FAO that does not include spinup
#  to_delete <- paste0("/rd/gem/private/fishmip_inputs/ISIMIP3a/processed_forcings/fao_inputs_gridcell/ctrlclim/", degree, "/control_no_spinup_FAO_", this_FAO, "_", degree, ".csv")
 # unlink(to_delete)
  ...
  # lat, lon, FAO, Date, Year, Month are repeated so need to remove these column repetitions 
  output_obs_all_variables<-output_obs_all_variables[,c(1,2,3,4,5,6,7,14,21,28,35)]
  # add grid cell area and depth (set depth to 200m for all grid cells)
  output_obs_all_variables<-left_join(output_obs_all_variables, area_frame, by = c("lat" = "lat", "lon" = "lon"))
  
  if(!("depth" %in% colnames(output_obs_all_variables))){ # If no depth variable
  output_obs_all_variables<- output_obs_all_variables %>% arrange(lat, lon, Date) %>% dplyr::mutate(depth = 200)
  }
  ...
  output_obs_all_variables<-output_obs_all_variables %>% 
    dplyr::select(lat, lon, FAO, Date, Year, Month, area_m2, depth, `expc-bot`, tob, tos, sphy, lphy) %>% 
    dplyr::rename(lat = lat, lon = lon, t = Date, sbt = tob, sst = tos, expcbot = `expc-bot`)
  
  output_obs_all_variables<-output_obs_all_variables[,c("lat","lon","FAO", "t", "Year","Month","lphy", "sphy", "sbt", "sst", "depth", "area_m2", "expcbot")]
  
  # # dplyr method, CHECKED - WORKS FOR ME, RFH SEP 2023
  output_obs_all_variables<-output_obs_all_variables %>% ungroup() %>%
    dplyr::mutate(er = getExportRatio(sphy,lphy,sst,depth),
                  er = ifelse(er<0,0, ifelse(er>1,1,er)),
                  er = round(er, 3),
                  sst = round(sst, 3),
  ...
  # write output files 
  this_destination_path_obs <- paste0("/rd/gem/private/fishmip_inputs/ISIMIP3a/processed_forcings/fao_inputs_gridcell/obsclim/", degree, "/observed_historical_FAO_", this_FAO, "_", degree, ".csv")
...
#  this_destination_path_obs <- paste0("/rd/gem/private/fishmip_inputs/ISIMIP3a/processed_forcings/fao_inputs_gridcell/obsclim/", degree, "/observed_FAO_", this_FAO, "_", degree, ".csv")
...
#  to_delete <- paste0("/rd/gem/private/fishmip_inputs/ISIMIP3a/processed_forcings/fao_inputs_gridcell/obsclim/", degree, "/observed_no_spinup_FAO_", this_FAO, "_", degree, ".csv")
#  unlink(to_delete)
